...
export const classifyCurrentStage = (heartRates, hrvs) => {
    ...
    if (currentHr >= median)
        return 'awake';
    if (currentHr <= low && currentHrv < hrvHigh)
        return 'deep';
    if (currentHrv >= hrvHigh)
        return 'rem';
    return 'light';
};
const applyStageTemperature = async (side, settingsData) => {
    ...
    const heartRates = recentVitals.map((v) => v.heart_rate).filter((v) => v !== null);
    const hrvs = recentVitals.map((v) => v.hrv).filter((v) => v !== null);
...
export const scheduleSleepStageTemperatures = (settingsData) => {
...
//# sourceMappingURL=sleepStageTemperatureScheduler.js.map